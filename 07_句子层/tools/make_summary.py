...
w('缺的不是零星几条，而是有规律的六族（族内代表见括号，逐条见各篇 `clauses.jsonl`）：')
w()
w('| 族 | 机制 | 条数 | 代表 | 能否并进现有情况 |')
w('|---|---|---|---|---|')
w('| 甲 两处拼一句 | 不相邻的两处各取半句，补一个动词粘起来 | ~18 | 让制造强国梦后继有人；南沙引来香港科技大学（广州） | **能**，单开 C13，见第 8 节 |')
w('| 乙 概括成对仗短语 | 一整段做法压成自拟的四到八字口号 | ~7 | 经验是：人文聚人心，绿色聚人气；抢占新赛道新高地 | 不宜，**本质是自造，归 Z 更诚实** |')
w('| 丙 引语／设问改判断句 | 换主语、疑问改陈述、陈述改要求 | ~6 | 长远看，它是留给子孙的"绿色银行" | **能**，把 C2 说明扩成「截半句／转述／改判断句」 |')
w('| 丁 判断句调序＋自造判断框 | 材料判断拆开重排，或只借一个词 | ~6 | 人往产业走，是乡村最朴素的规律 | **拆开**：纯调序并进 C1，自造框归 Z |')
w('| 戊 主客翻转／判断改使令 | 让 X 当 Y；动宾对调 | 2 | 让企业当创新主角 | 不宜，手册已判为低性价比写法（X7） |')
w('| 己 标题 | 标题本就是七处自造的第一处 | 3 | 以绿美广东为笔　绘就……的画卷 | 不必，**标题属自造清单，不该用改法表描述** |')
w()
# ── 3
w('## 3. 哪几种情况最值得先学')
w()
